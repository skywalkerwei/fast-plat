...
class Response
{
     ...
     */
    public function fail(string $message = '', int $code = HttpResponse::HTTP_BAD_REQUEST, $data = null, array $header = [], int $options = 0)
    {
        response()->json(
    ...
    {
        return response()->json($this->formatData($data, $message, $code), 200, $headers, $option);
    }
    ...
    protected function formatData($data, $message, &$code)
    {
        ...
        //{success:true|false, data:[]|{} [, code:, msg:] }
        return [
            'success' => $status,
    ...
    protected function formatPaginatedResourceResponse($resource, string $message = '', $code = HttpResponse::HTTP_OK, array $headers = [], $option = 0)
    {
        ...
        return tap(
            response()->json($this->formatData($data, $message, $code), 200, $headers, $option),
    ...
    {
        return tap(
            response()->json($this->formatData($this->parseDataFrom($resource), $message, $code), 200, $headers, $option),
            function ($response) use ($resource) {
